...
function App(){
 ...
 const minimize=app=>{setWindows(ws=>ws.map(x=>x.app===app?{...x,minimized:true}:x));setActive(a=>a===app?null:a)};
 const toggleFullscreen=()=>{if(!document.fullscreenElement)document.documentElement.requestFullscreen?.().then(()=>setFull(true)).catch(()=>setFull(true));else document.exitFullscreen?.().then(()=>setFull(false)).catch(()=>setFull(false))};
 ...
 return <div className={"os "+(dark?"dark":"light")} onClick={()=>{setContext(null);setMenu(null)}} onContextMenu={e=>{e.preventDefault();setContext({x:e.clientX,y:e.clientY});setMenu(null)}}>
  ...
  {menu&&<MenuBarMenu type={menu} close={()=>setMenu(null)} open={open} closeApp={()=>active&&close(active)} minimize={()=>active&&minimize(active)} windows={windows} focus={focus} dark={dark} setDark={setDark} fullscreen={toggleFullscreen} mission={()=>setMission(true)} shutdown={()=>setShutdown(true)}/>}
  ...
  <main className="desktop" onDoubleClick={()=>setContext(null)}>
   ...
   {windows.filter(w=>w.space===space).map(w=><Window key={w.id} data={w} active={active===w.app} onFocus={()=>focus(w.app)} onClose={()=>close(w.app)} update={patch=>setWindows(ws=>ws.map(x=>x.id===w.id?{...x,...patch}:x))} dark={dark} setDark={setDark}/>)}
  </main>
...
function MenuBarMenu({type,close,open,closeApp,minimize,windows,focus,dark,setDark,fullscreen,mission,shutdown}){const items={apple:[["About This Mac",()=>open("System")],["System Settings…",()=>open("Settings")],["Sleep",()=>{}],["Restart…",()=>location.reload()],["Shut Down…",shutdown]],mako:[["About makOS",()=>open("System")],["System Settings…",()=>open("Settings")]],file:[["New Finder Window",()=>open("Finder")],["Open…",()=>open("Finder")],["Close Window",closeApp]],edit:[["Undo",()=>{}],["Cut",()=>{}],["Copy",()=>{}],["Paste",()=>{}]],view:[["Show All Windows",()=>windows.forEach(w=>focus(w.app))],["Mission Control",mission],["Enter Full Screen",fullscreen]],window:[["Minimize",minimize],["Bring All to Front",()=>windows.forEach(w=>focus(w.app))]],help:[["makOS Help",()=>open("Safari")],["Parts & Service",()=>open("PartsService")]]};return <div className="topMenu" onClick={e=>e.stopPropagation()}><div className="topMenuInner">{(items[type]||[]).map(([label,fn])=><button key={label} onClick={()=>{fn();close()}}>{label}<span/></button>)}</div></div>}
